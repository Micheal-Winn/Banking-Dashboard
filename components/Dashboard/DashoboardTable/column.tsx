...
import { activeStatus } from "@/components/TableData/data"
...
import { DataTableRowActions } from "@/components/TableComponents/data-table-row-actions";
...
export const columns: ColumnDef<DashboardCustomer>[] = [
    ...
    },
    {
        ...
        header:"Status",
        cell:({row})=> {
            const status = activeStatus.find(status => status.label === row.getValue("status"))
            if (!status) {
                return null;
            }
            return (
